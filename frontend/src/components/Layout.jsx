...
const nav = [['/', LayoutDashboard, 'Visão geral', 'dashboard.view'], ['/mesas', UtensilsCrossed, 'Salão & mesas', 'tables.view'], ['/kds', ChefHat, 'Produção KDS', 'kds.view'], ['/pedidos', ShoppingBag, 'Pedidos', 'orders.view'], ['/estoque', Package, 'Estoque', 'stock.view'], ['/financeiro', WalletCards, 'Financeiro', 'finance.view'], ['/clientes', Users, 'Clientes', 'customers.view'], ['/cardapio-admin', BookOpen, 'Cardápio', 'menu.view'], ['/relatorios', ChartNoAxesCombined, 'Relatórios', 'reports.view']];
...
export default function Layout() {
  ...
  return <div className="app-shell">
    ...
    <main className="main-area"><header><button className="mobile-menu" aria-label="Abrir menu" onClick={() => setMenuOpen(true)}><Menu /></button><div className="search"><Search /><input placeholder="Buscar na operação..." aria-label="Buscar na operação" /></div><div className="header-actions"><NavLink className="icon-btn desktop-action" to={`/cardapio/${data.settings.slug || ''}`} title="Cardápio público"><BookOpen /></NavLink>{can(user, 'tables.view') && <button className={`icon-btn desktop-action notification-button ${calls ? 'has-alert' : ''}`} title={`${calls} chamado(s) pendente(s)`} onClick={() => navigate('/mesas')}><Bell />{calls > 0 && <em>{calls}</em>}</button>}{can(user, 'orders.edit') && <button className="primary" onClick={() => navigate('/pedidos?novo=1')}><Plus /> <span>Novo pedido</span></button>}</div></header><div className="content"><Outlet /></div></main>
  </div>;
}
