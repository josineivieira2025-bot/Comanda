...
import { Plus, WalletCards, LockKeyhole, Download, DoorOpen, Banknote, QrCode, CreditCard, CircleDollarSign, ArrowDownLeft, ArrowUpRight, ReceiptText, CheckCircle2 } from 'lucide-react';
...
export default function Finance() {
  const { data, mutate, cashAction, payReceivable, can } = useApp();
  ...
  async function receive() { await payReceivable(receiving.id, paymentMethod); setReceiving(null); }
  ...
  return <>
    ...
    <Modal open={!!receiving} onClose={() => setReceiving(null)} title={`Receber mesa ${receiving?.table?.number || ''}`} subtitle="CONFIRMAR PAGAMENTO"><div className="payment-summary"><span>Comanda #{receiving?.number}</span><strong>{money(receiving?.total)}</strong></div><label>Forma de pagamento<select value={paymentMethod} onChange={event => setPaymentMethod(event.target.value)}><option value="PIX">PIX</option><option value="CASH">Dinheiro</option><option value="CARD">Cartão</option><option value="VOUCHER">Voucher</option></select></label>{!data.financeSession && <div className="form-warning">Abra o caixa antes de confirmar o recebimento.</div>}<div className="modal-footer"><button className="secondary" onClick={() => setReceiving(null)}>Cancelar</button><button className="primary" disabled={!data.financeSession} onClick={receive}><CheckCircle2 /> Confirmar e liberar mesa</button></div></Modal>
