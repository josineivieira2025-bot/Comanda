...
export default function Tables() {
  ...
  return <>
    ...
    <div className="tables-grid">{data.tables.map(table => <article className={`table-card ${table.status}`} key={table.id}><div className="table-card-head"><div><small>MESA</small><h2>{table.number}</h2></div><StatusBadge status={table.status} /></div><div className="table-card-body"><span>{table.seats} lugares</span><b>{money(tableTotal(table.id))}</b><small>{table.calls?.[0] ? (table.calls[0].type === 'BILL' ? 'Cliente solicitou a conta' : 'Cliente chamou o garçom') : table.note || 'Sem observações'}</small></div>{table.calls?.length > 0 && <button className="call-alert" onClick={() => resolveCall(table.calls[0].id)}>Atender chamado</button>}<div className="card-actions"><button onClick={() => toggle(table)}>{table.status === 'available' ? <><DoorOpen /> Abrir comanda</> : <><ReceiptText /> Fechar e receber</>}</button><button aria-label="Editar mesa" onClick={() => open(table)}><Pencil /></button><button aria-label="Excluir mesa" onClick={() => confirm('Excluir esta mesa?') && remove('tables', table.id)}><Trash2 /></button><a aria-label="Abrir portal da mesa" href={`/mesa/${table.id}`} target="_blank" rel="noreferrer"><ExternalLink /></a></div></article>)}</div>
